...
const LEVEL_PLAYLIST_REGEX_SLOW = new RegExp(
  [
    /#EXT-X-(PROGRAM-DATE-TIME|BYTERANGE|DATERANGE|DEFINE|KEY|MAP|PART|PART-INF|PLAYLIST-TYPE|PRELOAD-HINT|RENDITION-REPORT|SERVER-CONTROL|SKIP|START|SEMANTICLEVEL|SEMANTICTYPE|PRIVACY|NEXT-SEMANTICLEVEL):(.+)/
      .source, // UPDATE SEMANTICLEVEL TAGS
    ...
    while ((result = LEVEL_PLAYLIST_REGEX_FAST.exec(string)) !== null) {
      ...
      if (duration) {
        ...
        switch (tag) {
          ...
          // Add for SEMANTICLEVEL TAGE
          case 'SEMANTICLEVEL': {
            const semanticLevel = parseFloat(value1);
            if (Number.isFinite(semanticLevel)) {
              frag.semanticLevel = semanticLevel;
              frag.tagList.push(['EXT-X-SEMANTICLEVEL', value1]);
            } else {
              logger.warn(`Invalid EXT-X-SEMANTICLEVEL tag: "${value1}"`);
            }
            break;
          }
          case 'SEMANTICTYPE': {
            const semanticType = parseFloat(value1);
            if (Number.isFinite(semanticType)) {
              frag.semanticType = semanticType;
              frag.tagList.push(['EXT-X-SEMANTICTYPE', value1]);
            } else {
              logger.warn(`Invalid EXT-X-SEMANTICTYPE tag: "${value1}"`);
            }
            break;
          }
          case 'PRIVACY': {
            const privacy = parseFloat(value1);
            if (Number.isFinite(privacy)) {
              frag.privacy = privacy;
              frag.tagList.push(['EXT-X-PRIVACY', value1]);
            } else {
              logger.warn(`Invalid EXT-X-PRIVACY tag: "${value1}"`);
            }
            break;
          }
          case 'NEXT-SEMANTICLEVEL': {
            const nextSemanticLevel = parseFloat(value1);
            if (Number.isFinite(nextSemanticLevel)) {
              frag.nextSemanticLevel = nextSemanticLevel;
              frag.tagList.push(['EXT-X-NEXT-SEMANTICLEVEL', value1]);
            } else {
              logger.warn(`Invalid EXT-X-NEXT-SEMANTICLEVEL tag: "${value1}"`);
            }
            break;
          }
